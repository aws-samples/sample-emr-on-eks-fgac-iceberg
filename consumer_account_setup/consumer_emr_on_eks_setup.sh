...
export S3_PRODUCER_BUCKET=blog-emr-eks-fgac-data-$PRODUCER_AWS_ACCOUNT-$AWS_REGION-$ENVIRONMENT


export EMR_VC_NAME=emr-on-eks-$EKSCLUSTER_NAME
export EMR_LF_USER_NAMESPACE=lf-fgac-user
export EMR_LF_SECURE_NAMESPACE=lf-fgac-secure
...
###################################################################################
#                STEP 1 - Create Glue Database in Consumer account
###################################################################################
echo "==========================================================="
echo "  Create local Glue database......"
echo "==========================================================="
...
###################################################################################
#                 STEP 2 - create S3 bucket for EKS job script and spark logs
###################################################################################
...
##############################################################################################################
#      STEP 3 - Create query engine iam role used by Spark run in the Secure namespace
###############################################################################################################

echo "==================================================================================="
echo " Create EMR on EKS Query Engine IAM Role Tied to Secure Space Components....."
echo "==================================================================================="
    ...
    "Statement": [
        {
            ...
            "Resource": "*"
                ...
                {
            "Sid": "AssumeJobRoleWithSessionTagAccessForSecureDriver",
            "Effect": "Allow",
            "Action": [
                "sts:AssumeRole",
                "sts:TagSession"
        ...
        {
            "Sid": "AssumeJobRoleWithSessionTagAccessForSecureExecutor",
            "Effect": "Allow",
            "Action": [
                "sts:AssumeRole"
            ],
...
##############################################################################################################
#      STEP 4 - Create two EMR on EKS job execution roles 
#      (https://docs.aws.amazon.com/emr/latest/EMR-on-EKS-DevelopmentGuide/creating-job-execution-role.html)
###############################################################################################################
echo "======================================================================="
echo "  Create EMR on EKS Job Execution IAM Role for Team 1, Team2 ......"
echo "======================================================================="
...
cat >/tmp/job-trust-policy.json <<EOL
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
for role_name in $TEAM1_JOB_ROLE_NAME $TEAM2_JOB_ROLE_NAME; do
  aws iam create-policy --policy-name $role_name-policy --policy-document file:///tmp/job-execution-policy.json
  aws iam create-role --role-name $role_name --assume-role-policy-document file:///tmp/job-trust-policy.json
  aws iam attach-role-policy --role-name $role_name --policy-arn arn:aws:iam::$CONSUMER_AWS_ACCOUNT:policy/$role_name-policy
done
...
echo "==============================================="
echo "  Set up role-based access control for EMR on EKS ......"
echo "==============================================="
...
# RBAC Permissions
# create k8s role and rolebinding in user and secure ns
for ns in $EMR_LF_USER_NAMESPACE $EMR_LF_SECURE_NAMESPACE; do
  kubectl -n $ns apply -f emr-containers-role.yaml
done
# create k8s cluster role and rolebinding
kubectl apply -f ./emr-containers-cluster-role.yaml

echo "==============================================="
echo "  Setup IRSA for 2 job exeuction roles and 1 query engine role in secure space......"
echo "==============================================="
# Setup IRSA trust policy
...
--role-name $QUERY_ROLE_NAME

###################################################################################
#        STEP 6 - Create resource link in Glue to enable cross-account access
###################################################################################
echo "======================================================================="
echo "  Create resource link to shared patients table from producer account......"
echo "======================================================================="
...
echo "====================================================================="
echo "  Create resource link to shared claims table from producer account......"
echo "====================================================================="
